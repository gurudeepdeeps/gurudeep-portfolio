...
3. Copy `.env.example` to `.env` and fill your Appwrite values.
4. Contents of `.env`:
...
VITE_APPWRITE_COLLECTION_PROJECTS=your_projects_collection_id
VITE_APPWRITE_COLLECTION_EXPERIENCE=your_experience_collection_id
VITE_APPWRITE_COLLECTION_ENQUIRIES=your_enquiries_collection_id
VITE_APPWRITE_BUCKET_ID=your_storage_bucket_id
```

5. ### Appwrite setup (required)

- In Appwrite -> Project -> Platforms, add `http://localhost:5173` and your Vercel domain.
- Create the Database/Collections/Bucket referenced by the `VITE_APPWRITE_*` env vars.
- Permissions: Projects/Experience readable by public; Enquiries creatable by public.

6. Install dependencies: `npm install`

7. Start the dev server: `npm run dev`

8. Deploy on Vercel: follow `DEPLOYMENT.md`.
